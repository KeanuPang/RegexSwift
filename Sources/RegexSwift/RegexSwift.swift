...
public struct RegexSwift: CustomStringConvertible, CustomDebugStringConvertible {
    // MARK: Initialisation
    ...
    public init(string pattern: String, options: NSRegularExpression.Options = []) throws {
    ...
    public func allMatches(in string: String) -> [MatchResult] {
        let matches = regularExpression
            ...
            .map { MatchResult(string, $0) }
        if let firstMatch = matches.first { RegexSwift._lastMatch = firstMatch }
        return matches
    }
...
///
/// - returns: `true` if the regular expression matches, otherwise `false`.
public func ~= (regex: RegexSwift, string: String) -> Bool {
...
///
/// - returns: `true` if the regular expression matches, otherwise `false`.
public func ~= (string: String, regex: RegexSwift) -> Bool {
    return regex.matches(string)
}

// MARK: Conformances

extension RegexSwift: Hashable {}

extension RegexSwift: Codable {
